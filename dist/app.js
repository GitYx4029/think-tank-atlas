...
function render(){
  ...
  $('#list').innerHTML=shown.length?shown.map(d=>`<article class="card"><small>${d.country} · ${d.type}</small><button data-profile="${d.id}"><h3>${esc(d.name)}</h3><h4>${esc(d.short)}</h4></button><p class="stance">${esc(d.stance)}</p><div class="tags">${tags(d.topics)}</div><p class="roleLine"><b>功能</b> ${esc(d.roles.join(' · '))}</p>${d.funding?`<p class="funding"><b>资金来源</b>${esc(d.funding)}</p>`:''}<div class="actions"><label><input data-compare="${d.id}" type="checkbox" ${st.compare.includes(d.id)?'checked':''}>加入比较</label><span><a href="${d.rss}" target="_blank" rel="noopener">RSS ↗</a><button data-profile="${d.id}">档案 ↗</button></span></div></article>`).join(''):'<div class="empty">没有匹配结果，请减少筛选条件。</div>';
